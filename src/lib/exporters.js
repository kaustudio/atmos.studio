...
/* ONE CSS FOR THE CLIPBOARD AND THE FILE (22.09.26, by request: "fix the mismatch"). This used to
   write its own flavour — `--garnet-1…5` under "Generated by Atmos Gallery" — while the export's CSS
   file wrote `--palette-garnet-01…05` under "Primitive palette", so a reader who copied today and
   downloaded tomorrow got tokens that did not match their own code. It is the file's builder now, over
   the same entries, honouring the same semantic-scaffold switch: copied or downloaded, the CSS is
   byte-identical. */
export function paletteCss(pal, semantic) {
  const entries = semantic ? semanticEntries(pal, pal.roles) : primitiveEntries(pal);
  return buildCssFile(pal, entries, semantic);
}
